...
N identical `(tool, args)` pairs in a row return an `"loop detected"` error
...
**Status:** MCP server is spec-compliant and the registration schema matches
OpenClaw's documented format, but end-to-end verification against a live
OpenClaw gateway is still pending.
